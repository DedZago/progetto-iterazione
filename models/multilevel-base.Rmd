...
# Modello lineare  a effetti misti (glmmLasso)
Modello a effetti misti con la variabile peak

```{r}


if(file.exists("fitglmmLasso.Rdata")){
    load("fitglmmLasso.Rdata")
} else{
  fitglmmLasso = glmmLasso(fix = f0 ~ cog_load + time + current + time:current + time2:current + syllable1 + syllable2 +
                                 syllable1:syllable2 + time:syllable1 + time:syllable2 + peak:time + peak:time2 + 
                                 peak:syllable1 + peak:syllable2 + peak:syllable1:syllable2,
                           rnd = list(subject = ~  1 + time:syllable1 + time:syllable2 + time:current),
                           data = datasq,
                           lambda = 10, #boh valore a caso intanto
                           control = list(index=c(1,2,3,4,NA,5),method="REML",print.iter=TRUE)) 
  
    save(fitglmmLasso, file="fitglmmLasso.Rdata")
}
...
```{r}
fitglmmLasso = glmmLasso(fix = f0 ~ cog_load + time + current  + time2:current + syllable1 + syllable2,
                           rnd = list(subject = ~  1 ),
                           data = datasq,
                           lambda = 10, #boh valore a caso intanto
                           control = list(index=c(1,2,3,4,NA,5),method="REML",print.iter=TRUE)) 
...
## GAM con effetti misti ##

Provo ad utilizzare un modello GAM con smoothing sulla variabile tempo, assieme ad intercetta e interazioni `time, time:syllable1, time:syllable2, time:current`,  specifiche per soggetto.

```{r gamm4_fit}
library(gamm4)
if(file.exists("fitgamm4.Rdata")){
    load("fitgamm4.Rdata")
} else{
    # Stima con gamm4
    ...
    # Equivalente di gamm4 con Stan
    fitGamm4 = stan_gamm4(f0 ~ cog_load + s(time) + current + time:current + syllable1 +
                          syllable2 + syllable1:syllable2 + time:syllable1 + time:syllable2,
                      random = ~ (1 + time:syllable1 + time:syllable2 + time:current|subject),
                      data = mandarino,
                      algorithm = "meanfield")
    save(fitGamm4, file="fitgamm4.Rdata")
}
```

```{r gamm4_pred}
# Predict per il modello con Stan oppure con gamm4
if("stanreg" %in% class(fitGamm4)){
    ypredGamm4 = posterior_predict(fitGamm4, draws = 100)
    ypred = apply(ypredGamm4, 2, mean)

} else{
    ypred = predict(fitGamm4)
}
ppc_subject("S1", mandarino, ypred)
ppc_subject("S2", mandarino, ypred)
ppc_subject("S3", mandarino, ypred)
