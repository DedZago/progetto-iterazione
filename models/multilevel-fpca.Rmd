...
#parametri da regolare per meanfield: tol_rel_obj (default 0.01), iter, elbo_samples (default 100), grad_samples (default 1)
#alla fine uso sampling con meno iterazioni
#PORCODDUE RICORDATI DI METTERE 4 CORES e magari 2 chains che poi magari non ha senso ma vabbè vediamo, ma quale cazzo è il default?

# Nel primo modello metto un po' di tutto come effetti fissi  ma tengo meno effetti casuali, e indipendenti
fitStanLmerFpca1 = stan_glmer(f0 ~ cog_load + current + syllable1 + syllable2 + syllable1:syllable2 +
                              ...
                              (1 | subject) +
                              (0 + current|subject) +
                              (0 + PC1|subject) + (0 + PC2|subject) + (0 + PC3|subject) + (0 + PC4|subject) #+
                              #(0 + PC1:syllable1 | subject) +
                              #(0 + PC1:syllable2 | subject) +
                              #(0 + PC2:syllable1 | subject) +
                              #(0 + PC2:syllable2 | subject) +
                              ...
                              ,
                         data = mandarino_fpca,
                    algorithm = "meanfield", tol_rel_obj = 0.0001, elbo_samples = 100, grad_samples = 3,
                    iter = 10000,
                    #chains = cores, cores = cores,
                    QR = TRUE)

save(fitStanLmerFpca1, file="fitStanLmerFpca1_sampl.Rdata") #questo è fatto con sampling con 600 iterazioni
save(fitStanLmerFpca1, file="fitStanLmerFpca1_var.Rdata")
#(300+300)
summary(fitStanLmerFpca1)
print(fitStanLmerFpca1)
