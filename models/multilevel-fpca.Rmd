...
```{r data_scramble}
mandarino_fpca = mandarino_fpca[permute(1:NROW(mandarino_fpca)), ]
...
```
```{r kfold_comparison}
rbind(cbind(mean(unlist(kf5)), sd(unlist(kf5))),
      cbind(mean(unlist(kf4)), sd(unlist(kf4))),
      cbind(mean(unlist(kf1)), sd(unlist(kf1))),
