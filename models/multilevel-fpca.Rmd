...
```
```{r kfold_comparison}
rbind(cbind(mean(unlist(kf5)), sd(unlist(kf5))/length(kf5)),
      cbind(mean(unlist(kf4)), sd(unlist(kf4))/length(kf4)),
      cbind(mean(unlist(kf1)), sd(unlist(kf1))/length(kf1)),
      cbind(mean(unlist(kf)), sd(unlist(kf))/length(kf)),
      cbind(mean(unlist(kf2)), sd(unlist(kf2))/length(kf2)),
      cbind(mean(unlist(kf3)), sd(unlist(kf3))/length(kf3))
      ) %>%
    set_rownames(c("Eff. misti syllable e PCA:syllable, full fissi",
                   "Eff. misti PCA:syllable, full fissi",
                   "No eff. misti PCA:syllable, full fissi",
